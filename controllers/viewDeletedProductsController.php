...
$getDeletedProducts = $product->getDeletedProductsByUser($userId);

// Validar ejecución correcta del método (si falla, cerrar la conexión y retornar response array de error)
if (!$getDeletedProducts['success']) {
    $database->closeConnection();
    die($getDeletedProducts['errorMessage']);
}

// Si todo salió bien, cerrar conexión y obtener array de productos
$database->closeConnection();
$allProducts = $getDeletedProducts['allProducts'];

// Incluir la vista
require_once '../views/viewDeletedProducts.php';
?>